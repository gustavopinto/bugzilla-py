...
class Bug:
  ...
  def __init__(self, status, reported_date, modified, component, version, platform, importance, flags, cc_list, comments, reporter, depends, blocks, attachment, reopened):
  ...
  def __str__(self):
    return self.status + "," + self.reported_date  + "," + self.modified + "," + self.component + "," + self.version + "," + self.platform + "," + self.importance + "," + self.flags + "," + self.cc_list + "," + str(self.comments) +"," + "https://bugzilla.mozilla.org/show_bug.cgi?id=" + self.id + "," + "," + self.reporter + "," + to_bin(self.depends) + "," + to_bin(self.blocks) + "," + self.attachment.attempts + "," + self.attachment.locs() + "," + self.reopened
...
def download(bug_id):
  ...
  return Bug(remove_html(status), remove_html(reported), remove_html(modified), remove_html(component), remove_html(version), remove_html(platform), remove_html(importance), remove_html(flags), remove_html(cc_list), comments, remove_html(reporter), depends, remove_html(blocks), attachment, reopened)
